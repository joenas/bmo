...
//
// 2011-01-26: 
// Added $sourcebaseUrl which makes it possible to set which baseUrl to use. This makes it
...
// Which directory to use as baseUrl for file listning, end with separator.
// Default is current directory
$baseUrl = "." . $SEPARATOR;
if(isset($sourcebaseUrl)) {
	$baseUrl = $sourcebaseUrl . $SEPARATOR;
}
...
$source_fullpath1 	= realpath($baseUrl);
$source_fullpath2 	= realpath($baseUrl . $source_currentdir);
...
if(isset($_GET['file'])) {
	...
	if(in_array($source_extension, $IMAGES)) {
		$source_content = "</div><div style='overflow:auto;'  class='secondary sourceimg'><img src='{$sourceSubDir}{$source_currentdir}/{$source_file}' alt='[image not found]' ></div>";
...
<div class='sourcecode secondary'>
	<h1>Visa källkod</h1>
